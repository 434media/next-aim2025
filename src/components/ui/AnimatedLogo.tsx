...
const additionalPartners = [
  ...
  { name: "711th Human Performance Wing", src: "https://ampd-asset.s3.us-east-2.amazonaws.com/Sponsor+Logos/711+(1).png" },
]
...
const LinkPreview: React.FC<LinkPreviewProps> = ({ children, href, description }) => {
  ...
  return (
    ...
    >
      <Link
        href={href}
        className="font-bold text-orange-600 hover:underline focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 rounded-sm"
      >
        {children}
      </Link>
      <AnimatePresence>
        {isHovered && (
          <motion.span
            ...
            className="absolute z-10 left-1/2 transform -translate-x-1/2 mt-2 w-64 bg-white rounded-lg shadow-lg p-4 text-sm text-gray-700"
          >
            {description}
          </motion.span>
        )}
    ...
    <section ref={containerRef} className="relative w-full overflow-hidden bg-white/5 py-20 sm:py-28">
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-12 justify-items-center mb-16"
        >
        ...
        <motion.div style={{ y: additionalPartnersY }} className="pt-16 border-t border-gray-200">
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 gap-6 md:gap-8 justify-items-center">
      ...
      <div
        className = "absolute inset-0 -z-10 flex items-start justify-start" > <GameOfLife />
      </div>
    </section>
    </>
  )
